...
cache_daily_files_1 = function (folder = "data", folder_save_to = "data"){
  files = list.files(here::here(folder)) %>%
    ...
    gsub("_.*", "\\1", .) %>%
    unique() %>%
    .[!is.na(as.numeric(.))] %>% .[as_date(.) != Sys.Date()]
...
send_files_to_gdrive = function(folder_with_files){

  files = here::here(folder_with_files) %>%
    list.files()

  paste0(folder_with_files, files)

  files %>%
    map(~{
      # dri ve_put(
        here::here(folder_with_files, paste0(.x, ".csv"))
